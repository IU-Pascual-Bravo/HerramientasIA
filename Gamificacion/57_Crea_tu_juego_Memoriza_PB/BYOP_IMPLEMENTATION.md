...
### 🔒 Seguridad en Descargas de Juegos Offline

Cuando se permite descargar el juego como HTML para uso offline, es **crítico** proteger la API key:

1. **Limpieza del Input**: Vaciar el campo de entrada y sus atributos HTML.
2. **Conversión de Imágenes a Base64**: Las URLs de imágenes contienen la API key como parámetro. Convertirlas a Base64 elimina esta exposición.
3. **Limpieza de localStorage**: Agregar un script que elimine la clave del localStorage al cargar el HTML descargado.
4. **Deshabilitar Botones**: Ocultar o deshabilitar botones de "Obtener API Key" que no funcionan offline.

```javascript
// Ejemplo de limpieza en la descarga
const apiKeyInputClone = docClone.querySelector('#apiKeyInput');
if (apiKeyInputClone) {
    apiKeyInputClone.value = "";
    apiKeyInputClone.setAttribute('value', '');
    apiKeyInputClone.placeholder = "API Key eliminada por seguridad";
}

// Convertir imágenes a Base64
const allImages = docClone.querySelectorAll('img');
...
// Script de limpieza para el HTML descargado
const cleanupScript = docClone.createElement('script');
...
`;
docClone.body.insertBefore(cleanupScript, docClone.body.firstChild);
```
